...
(() => {
  ...
  // Arriving from the Chat page's Visitor column (/admin/?visitor=…) follows that
  // person from the start; the address is put back so a reload shows everyone.
  const asked = new URLSearchParams(location.search).get('visitor');
  if (asked) { followed = asked; renderFollowed(); history.replaceState(null, '', location.pathname); }
