...
search-index.json               What the palette (⌘K) searches: pages, sections, case studies, posts (generated by blog.py)
...
- **Steps** (`initSteps`, after diabrowser.com). A decisions block marked
  ...
  Cross-Sell's decisions use it; any case study can, by adding the attribute.
...
⌘K (Ctrl K on Windows and Linux), or the Search row `main.js` adds to the
bar (`initPalette`), opens a dialog over the page: one field, and the pages,
sections, case studies and posts that match from `search-index.json`, which
`blog.py` writes from `index.html`, `work.html` and the posts (a case study's
own text stays out: the pages are gated). Arrows move, Enter goes, Escape
closes and focus returns. The last row hands whatever was typed to the
assistant, so a question the index cannot answer still gets one. The index
loads on first open, so a reader who never presses the keys pays nothing.

## Themes

Every colour is a token at the top of `styles.css`, set twice: the paper
register on `:root`, and the dark register (warm near-black, after
cursor.com) under `@media (prefers-color-scheme: dark)` for a root with no
choice stamped and under `:root[data-theme="dark"]` for one that has. The
footer's switch (`initTheme`) stamps `data-theme` and keeps it in
...
The dashboard is pinned to paper (`data-theme="light"` on its root).
...
small); the mono labels (DM Mono, `--font-mono`) take the micro role, the
facts' keys and a figure's number, so a label reads as the system's annotation
on the page rather than as more copy. Every captioned figure on a page is
numbered in order (`fig. 01`, a CSS counter).
