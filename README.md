...
- A work-index thumbnail shows the one piece of the design the case study is
  ...
  Ethereum keeps the plain `.wide` ground. Every ground fades what sits on it
  over the last stretch before each edge (`--fade`, 40 to 80 px): the
  `.case-media::after` overlay paints the ground itself, masked to a band along
  the edges, so a screen that runs off the ground dissolves into it instead of
  stopping at the clip line, and nothing inside the band is meant to be read.
